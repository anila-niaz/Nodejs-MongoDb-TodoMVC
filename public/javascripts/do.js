    ...
    var app = {
        ...
        actions: function(){
            $('#login-submit').live('click', function(){
                ...
                function authenticatioSuccessCallback(){
                    $('#login-form').hide();
                    $('#user-data').show();
                    $('#user-name').text(username);
                }
            });
            $('#option-available').live('click', function(){
                connection.updateAvailability("available");
            });

            $('#option-unavailable').live('click', function(){
                connection.updateAvailability("unavailable");
            });
        ...
        socketActions: function(){
            ...
            socket.on('users', function(data){
                if (!connection.getAcision())
                    return;

                users = connection.getPresentitiesFor(data, presentitiesSetCallback, presentitiesUpdatedCallback);
            });

            function presentitiesSetCallback(data){
                users = data;
                updateUserList();
            }

            function presentitiesUpdatedCallback(presentities){
                users.forEach(function(user){
                    for(var i = 0; i < presentities.length; i++)
                        if(user.address == presentities[i].user)
                            user.status = presentities[i].fields.state;
                });
            ...
            function updateUserList(){
                $('#user-list').html('');
                $('#user-list').append('<tr><th class="name">Name</th><th class="status">Status</th></tr>');
                for(var i = 0; i < users.length; i++){
                    $('#user-list').append('<tr class="user"><td>' + users[i].name +'</td>' +
                        '<td class="' + users[i].status + '">' + users[i].status + '</td>' +
                        '<td><input type="button" value="Call" class="call-user"/></td></tr>');
                };
            }
        },
